...
const BlogsPage: React.FC = () => {
    ...
    const fetchBlogs = async () => {
        setLoading(true);
        try {
            const response = await axios.get<{ data: Blog[] }>("https://farmigo-server.onrender.com/api/v1/blogs");
    ...
    const fetchCategories = async () => {
        setLoading(true);
        try {
            const response = await axios.get<Category[]>("https://farmigo-server.onrender.com/api/v1/categories");
    ...
    const handleCategoryChange = (
        ...
        blogState: NewBlog | Blog,
        setBlogState: React.Dispatch<React.SetStateAction<NewBlog | Blog | null>>
    ) => {
        const selectedCategories = Array.from(e.target.selectedOptions, (option) => option.value);
        setBlogState({ ...blogState, categories: selectedCategories });
    };
            ...
            <form onSubmit={handleCreateBlog} className="mb-10 bg-white p-6 rounded-lg shadow-lg max-w-xl mx-auto">
                ...
                <div className="mt-4">
                    ...
                    <select
                        ...
                        onChange={(e) => handleCategoryChange(e, newBlog, setNewBlog)}
                        className="border border-gray-300 rounded-lg p-2 w-full focus:ring-2 focus:ring-indigo-400"
                    >
                        {categories.length > 0 ? (
                ...
                <form onSubmit={handleUpdateBlog} className="mt-10 bg-white p-6 rounded-lg shadow-lg max-w-xl mx-auto">
                    ...
                    <div className="mt-4">
                        ...
                        <select
                            ...
                            onChange={(e) => handleCategoryChange(e, editingBlog, setEditingBlog)}
                            className="border border-gray-300 rounded-lg p-2 w-full focus:ring-2 focus:ring-indigo-400"
                        >
                            {categories.length > 0 ? (
                                categories.map((category) => (
